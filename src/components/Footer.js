...
import { site, footer } from '@/data/siteContent'
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-10 mb-10">
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2">
              {['Lead Response', 'Appointment Booking', 'Process Automation', 'WhatsApp Support', 'Custom Setup'].map((s) => (
                <li key={s}>
                  <a href="#services" className="text-white/60 hover:text-sand text-sm transition-colors">{s}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2">
              ...
              <li><a href={site.whatsappUrl} target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-sand text-sm transition-colors">WhatsApp Chat</a></li>
              <li><a href="#booking" className="text-white/60 hover:text-sand text-sm transition-colors">Book Consultation</a></li>
            </ul>
          </div>
        </div>
